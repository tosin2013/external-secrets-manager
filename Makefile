...
GIT_URL := https://github.com/tosin2013/external-secrets-manager.git
TAG := 0.1.0 
INSTALL_PATH = ~/.ansible-navigator.yml
SOURCE_FILE = ~/external-secrets-manager/ansible-navigator/release-ansible-navigator.yml

INSTALL_ANSIBLE_NAVIGATOR := pip3 install ansible-navigator
BUILD_CMD := tag=$(TAG) && cd ~/external-secrets-manager/ansible-builder/ && ansible-builder build -f execution-environment.yml -t external-secrets-manager:$${tag} -v 3
...
REMOVE_IMAGES := podman rmi $$(podman images | grep "external-secrets-manager" | awk '{print $$3}')
